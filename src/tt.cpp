...
// Unknown score constant used when a read fails
...
void init_tt(int megabytes) {
    ...
    tt_size = hash_size / sizeof(TT_Entry);
...
int read_tt(int depth, int alpha, int beta, int* best_move) {
    ...
    TT_Entry* entry = &tt[hash % tt_size]; // Instant index lookup
    
    // Verify it is the exact same board position
    if (entry->hash_key == hash) {
        // Always extract the best move (great for move ordering!)
        *best_move = entry->best_move;
        
        // Only trust the score if the previous search was deep enough
...
void write_tt(int depth, int score, int flag, int best_move) {
    U64 hash = generate_hash_key();
    TT_Entry* entry = &tt[hash % tt_size]; // Overwrite scheme: Always replace
