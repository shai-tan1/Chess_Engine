...
#include "board.h"
#include "movegen.h"
#include "perfit.h"  // Assuming you named it perft.h!
#include "search.h" // ADDED: Your AI Brain
#include "uci.h"
...
int main() {
    ...
    // parse_fen(start_position);
    // 1. Set the standard starting position
    // string start_position = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1";
    // // string start_position = "k7/8/8/8/8/8/1R6/2R4K w - - 0 1";
    // parse_fen(start_position);
    //
    // // 2. Run the Benchmark
    // perft_test(5);
    ...
    //
    //     // Check for checks
    //     if (const int current_king = get_lsb_index(side == white ? bitboards[K] : bitboards[k]); is_square_attacked(current_king, side == white ? black : white)) {
    //         cout << "\n[!] YOUR KING IS IN CHECK!\n";
    //     }
    //
    //     string source_str, target_str;
    //
    //     // --- ADDED: 'go' COMMAND INTERCEPT ---
    //     cout << "\nEnter source square (e.g. g1) or type 'go' for AI: ";
    //     cin >> source_str;
    //
    //     if (source_str == "go") {
    //         cout << "\nThinking...\n";
    //         int score = search_position(5); // Search 5 plies deep
    //
    //         cout << ">>> Engine Evaluation: " << score << " centipawns\n";
    //
    //         if (best_move != 0) {
    //             make_move(best_move);
    //             cout << ">>> AI plays its move!\n";
    //         } else {
    //             cout << ">>> AI has no legal moves (Checkmate or Stalemate).\n";
    //         }
    //         continue; // Skip the rest of the loop and redraw the board
    //     }
    //
    //     cout << "Enter target square (e.g. f3): ";
    //     cin >> target_str;
    //
    //     const int source = parse_square(source_str);
    //     const int target = parse_square(target_str);
    //
    ...
    //         if (const int move = move_list.moves[i]; GET_MOVE_SOURCE(move) == source && GET_MOVE_TARGET(move) == target) {
    ...
    //         move_exists = false;
    //         for (int i = 0; i < move_list.count; i++) {
    //             if (const int move = move_list.moves[i]; GET_MOVE_SOURCE(move) == source && GET_MOVE_TARGET(move) == target && GET_MOVE_PROMOTED(move) == user_promo_piece) {
    //                 move_to_make = move;
    //                 move_exists = true;
    //                 break;
    //             }
